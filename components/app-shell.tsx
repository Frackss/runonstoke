...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-transparent text-white">
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-zinc-950/72 px-4 py-2 backdrop-blur-xl lg:hidden">
        ...
        {navItems.map(({ label, href, Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                "flex min-h-11 flex-col items-center justify-center gap-1 rounded-xl px-2 py-2 text-[11px] text-zinc-500 transition",
                active && "bg-emerald-300/12 text-emerald-300",
              )}
            >
              <Icon className="size-4" />
              {label}
